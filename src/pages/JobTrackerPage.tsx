...
function JobRow({ job, onAsk }: { job: TrackedJob; onAsk: () => void }) {
  ...
  return (
    <li className="flex flex-wrap items-center gap-x-3 gap-y-2 px-4 py-3 transition-colors hover:bg-surface-hover">
      {/* Company tile, where LinkedIn shows the company logo */}
      <span
        className="grid h-12 w-12 shrink-0 place-items-center rounded border border-line bg-surface text-lg font-semibold text-ink-muted"
        aria-hidden
      >
        {job.company[0]}
      </span>
      ...
      {hasConnections ? (
        ...
        <span
          className="ml-[60px] inline-flex shrink-0 cursor-not-allowed items-center sm:ml-0 rounded-full border border-line px-3.5 py-1.5 text-[12px] font-medium text-ink-faint"
